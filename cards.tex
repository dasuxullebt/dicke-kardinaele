...
{\bf Beweis:}
\begin{itemize}
  \item ``$\Rightarrow$'': Sei $\left<S_\alpha|\alpha\in C\right>$,
    $S_\alpha\subseteq\alpha$ abgeschlossen unbeschränkt in $\kappa$,
    gegeben. Ohne Einschränkung
    $C\subseteq\{\alpha<\kappa|\lim(\alpha)\}$. Definiere
...
(a)^{<\omega}_2$ für ein $\tau$. Sei $\kappa = \min\{\tau | \tau
...
\begin{itemize}
  \item[(1)] $I\subseteq C$: \newline Angenommen nicht. Für $\gamma\in
    ...
    \gamma\phantom{aa}\mathnode{m2}{\mbox{ da }}C\mbox{
      ...
      (b), so $\overline{I} = \{h(\gamma) | \gamma \in I\}$ Menge von
...
$\left<S_\alpha | \alpha < \tau\right>$ mit $S_\alpha \subseteq
\alpha$ existieren $\omega \le \alpha < \beta < \tau$ mit $S_\alpha =
S_\beta \cap \alpha$. Sei $A = \{\tau \in \Card | E(\tau)\} \neq
...
$\left<v_i|i\in\omega\right>$ feste Aufzählung der Variablen.
...
\begin{itemize}
  ...
  \item[(E3)] $\Sigma\supseteq\{v_i\in\On|i\in\omega\}\cup\{v_i\in
    v_j|i<j\}$
  \item[(E4)] $\Sigma\supseteq\{\varphi(v_{i_0},\ldots,v_{i_{n-1}})
    \leftrightarrow
    \varphi(v_{j_0},\ldots,v_{j_{n-1}})|\varphi(x)\phantom{a}
...
hierzu $\left<c_i|i\in\On\right>$ eine Klasse von neuen
Konstanten. Setze zuerst
$H'(\Sigma,\alpha)=\{t(c_{\xi_0},\ldots,c_{\xi_{n-1}})|
...
\{\eta_0,\ldots,\eta_{m-1}\}\stackrel{\sim}{\rightarrow} k$. Beachte,
dass $\sim$ nicht von $\alpha$ abhängt. $\sim$ ist Äquivalenzrelation
nach (E4). Setze $[t(\vec{c})]_\alpha=\{t'(\vec{c'})\in
H'(\Sigma,\alpha)|t(\vec{c})\sim_{\Sigma,\alpha} t'(\vec{c'})\}$ und
$H(\Sigma,\alpha)=\{[t(\vec{c})]_\alpha|t(\vec{c})\in
...
\stackrel{\sim}{\rightarrow} k$. Ist wohldefiniert wegen (E4). Setze
schließlich
$\mathfrak{H}(\Sigma,\alpha)=\left<H(\Sigma,\alpha),E_\alpha\right>$. Weiterhin
sei $I_\alpha=\{[c_\xi]_\alpha|\xi<\alpha\}$. Der Nachweis von (E5)
...
H_\beta|\mathfrak{H}_\beta\models b<[c_\alpha]_\beta\}$$ Insbesondere
ist also $[c_\alpha]_\beta$ in $\On^{\mathfrak{H}_\beta}$ das Supremum
von $\{[c_\gamma]_\beta|\gamma<\alpha\}$.
...
\begin{itemize}
  ...
  \item ``Insbesondere'': Folgt sofort, da wegen $\Sigma$ konfinal
    $\{[c_\gamma]_\beta|\gamma<\alpha\}$ konfinal in
    $i_{\alpha\beta}''\On$.
\end{itemize}
\hfill $\square$
...
\begin{itemize}
  ...
  \item (c)$\Rightarrow$(a): indirekt: Sei etwa
    ...
    \bigcup\limits_{n=0}^{\infty}\{\xi_0^n,\ldots,\xi_{i_n}^n\}\stackrel{\sim}{\rightarrow}\alpha$. Also
...
$\{\omega_{n+1}|n\in\omega\}\subseteq I$. Hieraus folgt die
Behauptung.
\hfill $\square$
...
[hierbei: $H_{\omega_1}=\{x|\TC(x)\mbox{ ist abzählbar}\}$.]
...
$\otp(I)=\omega_1$. Sei $\left<\gamma_i|i<\omega_1\right>$ die
